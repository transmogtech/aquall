...
const DataTable = ({ changeStatusAppClassifiedImage, deleteAppClassifiedImage, getAppClassifiedImages, appClassifiedImage: { appclassifiedimages, loading } }) => {
  ...
  useEffect(() => {
    getAppClassifiedImages();
  }, []);


  appclassifiedimages.forEach(row => searchTable.push({ id: row._id, priority: row.priority, image: row.image, action: row._id, status: row.status, url: row.url, created: moment(row.created).format('MMMM Do YYYY, h:mm:ss a') }));
  ...
  const handleChageStatus = () => {
    changeStatusAppClassifiedImage(id, selectedSingle, comment);
        ...
        cell: (cell) => {
          return (
            <div>
              ...
              <Link to={`/edit/app-classified-image/${cell.getValue()}`} className="btn btn-sm btn-warning"><i className='las la-pen'></i></Link>&nbsp;&nbsp;
...
DataTable.propTypes = {
  getAppClassifiedImages: PropTypes.func.isRequired,
  appClassifiedImage: PropTypes.object.isRequired,
  deleteAppClassifiedImage: PropTypes.func.isRequired,
}

const mapStateToProps = state => ({
  appClassifiedImage: state.appClassifiedImage,
});

export default connect(mapStateToProps, { changeStatusAppClassifiedImage, deleteAppClassifiedImage, getAppClassifiedImages })(DataTable);