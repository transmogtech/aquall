...
const DataTable = ({ changeStatusSponsoredAd, deleteSponsoredAd, getSponsoredAds, sponsoredAd: { sponsoredads, loading } }) => {
  ...
  sponsoredads.forEach(row => searchTable.push({ id: row._id, priority: row.priority, image: row.image, action: [row._id, row.status], status: row.status, name: row.name, discount: row.discount, url: row.url, created: moment(row.created).format('MMMM Do YYYY, HH:mm:ss') }));
        ...
        cell: (cell) => {
          return (
            <div>
              ...
              <Link to={`/edit/sponsor-ad/${cell.getValue()[0]}`} className="btn btn-sm btn-warning"><i className='las la-pen'></i></Link>&nbsp;&nbsp;
