...
const DataTable = ({ changeStatusAppSliderImage, deleteAppSliderImage, getAppSliderImages, appSliderImage: { appsliderimages, loading } }) => {
  ...
  appsliderimages.forEach(row => searchTable.push({ id: row._id, category: row.categoryId.title, company: row.companyId.name, discount: row.discount, priority: row.priority, image: row.image, action: row._id, status: row.status, url: row.url, created: moment(row.created).format('MMMM Do YYYY, HH:mm:ss') }));
