...
const DataTable = ({ changeStatusAppSliderImage, deleteAppSliderImage, getAppSliderImages, appSliderImage: { appsliderimages, loading } }) => {
  ...
  appsliderimages.forEach(row => searchTable.push({ id: row._id, category: row.categoryId.title, company: row.companyId?.name, discount: row.discount, priority: row.priority, image: row.image, action: [row._id, row.status], status: row.status, url: row.url, created: moment(row.created).format('MMMM Do YYYY, HH:mm:ss') }));
  ...
  const handleChageStatus = () => {
    changeStatusAppSliderImage(id, selectedSingle, comment);
        ...
        cell: (cell) => {
          return (
            <div>
              ...
              <Link to={`/edit/app-slider-image/${cell.getValue()[0]}`} className="btn btn-sm btn-warning"><i className='las la-pen'></i></Link>&nbsp;&nbsp;
...
DataTable.propTypes = {
  getAppSliderImages: PropTypes.func.isRequired,
  appSliderImage: PropTypes.object.isRequired,
  deleteAppSliderImage: PropTypes.func.isRequired,
}

const mapStateToProps = state => ({
  appSliderImage: state.appSliderImage,
});

export default connect(mapStateToProps, { changeStatusAppSliderImage, deleteAppSliderImage, getAppSliderImages })(DataTable);