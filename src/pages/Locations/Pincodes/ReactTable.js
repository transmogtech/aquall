...
const DataTable = ({ getPincodes, deletePincode, changeStatusPincode, pincode: { pincodes, loading } }) => {
  ...
  const handleChageStatus = () => {
    changeStatusPincode(id, selectedSingle);
        ...
        cell: (cell) => {
          return (
            <div>
              ...
              <Link to={`/edit/pincode/${cell.getValue()[0]}`} className="btn btn-sm btn-warning"><i className='las la-pen'></i></Link>&nbsp;&nbsp;
...
export default connect(mapStateToProps, { getPincodes, deletePincode, changeStatusPincode })(DataTable);