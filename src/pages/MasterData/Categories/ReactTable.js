...
const DataTable = ({ getCategories, deleteCategory, changeStatusCategory, category: { categories, loading } }) => {
  ...
  categories.forEach(row => searchTable.push({ id: row._id, title: row.title, action: row._id, status: row.status, created: moment(row.created).format('MMMM Do YYYY, h:mm:ss a') }));
  ...
  const handleChageStatus = () => {
    changeStatusCategory(id, selectedSingle);
...
export default connect(mapStateToProps, { getCategories, deleteCategory, changeStatusCategory })(DataTable);